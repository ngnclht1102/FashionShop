...
function shortcode_testimonials($atts, $content = null, $code) {
	...
	if( $style == "1" || $style == "2" || $style == "3" || $style == "4" || $style == "5" || $style == "6")	:
		...
		if($testimonial_count > 0 ):
		$output .= '<div id="container" class="testimonials-container style-'.$style.'">';		
		if($type == "slider") { 
			if($testimonial_count > $items_per_column)
				$output .= '<div id="'.$items_per_column.'_testimonial_carousel" class="testimonial-carousel">';
			else
			...
			$output .= '<div id="testimonial_grid" class="masonry cols-'.$items_per_column.'">';			
		}
		$i = 1;
		foreach($testimonial_array as $post) : setup_postdata($post);	
			get_post_meta($post->ID, 'testimonial_position', TRUE) ? $testimonial_position = get_post_meta($post->ID, 'testimonial_position', TRUE) : $testimonial_position = '';
			...
			elseif($i % $items_per_column == 0)
				...
				$class = "";
			$output .= '<div class="item'.$class.'"><div class="product-block">';
				$output .= '<div class="single-testimonial" '.$color_style.'>';
				if($style == '1' || $style == '4'):			
					$output .= '<div class="testmonial-image">';
					if ( has_post_thumbnail() && ! post_password_required() ) :	
						$post_thumbnail_id = get_post_thumbnail_id();
						$post_thumbnail_url = wp_get_attachment_url( $post_thumbnail_id );
						$output .= '<a href='.$image_link_url.'><img src="'.mr_image_resize($post_thumbnail_url, $image_width, $image_height, true, 'left', false).'" title="'.get_the_title().'" alt="'.get_the_title().'"></a>';				else:
						$output .= '<i style="width:'.$image_width.';height:'.$image_height.';line-height:'.$image_height.'px;" class="fa fa-user"></i>';
					endif;	
					$output .= '</div>';
					$output .= '<div class="testmonial-text">';			
						$output .= '<div class="testimonial-title">'.get_the_title().'</div>';
						if(!empty($testimonial_position)):
							$output .= '<div class="testimonial-position" '.$color_style.'>'.$testimonial_position.'</div>';
						endif;														 
					$output .= '</div>';
					$output .= '<div class="testimonial-content">';
						$output .= '<div class="testimonial-top"'.$background_color_style.'><blockquote><q>'.arise_blog_post_excerpt(22).'</q></blockquote></div>';
						$output .= '<div class="testimonial-bottom"></div>';
					$output .= '</div>';
				endif;			
				if($style == '2' || $style == '3'):
					$output .= '<div class="testimonial-content-outer">';					
						$output .= '<div class="testmonial-image">';
							if ( has_post_thumbnail() && ! post_password_required() ) :	
								$post_thumbnail_id = get_post_thumbnail_id();
								$post_thumbnail_url = wp_get_attachment_url( $post_thumbnail_id );
						...
						$output .= '</div>';	
						$output .= '<div class="testimonial-content">';
		$output .= '<div class="testimonial-top" style="background-color:#'.$background_color.';"><blockquote><q>'.arise_blog_post_excerpt(22).'</q></blockquote></div>';
							$output .= '<div class="testmonial-text">';			
								$output .= '<div class="testimonial-title">'.get_the_title().'</div>';
								if(!empty($testimonial_position)):
									if($style == '3') 
		...
		$output .= '<div class="no-result">Please write style 1 or 2 or 3 or 4...</div>';
	endif;
	return $output;
}
add_shortcode('tm_testimonials', 'shortcode_testimonials');
   ...
   extract(shortcode_atts(array(
	...
	while ( $team_array->have_posts() ) : $team_array->the_post();
		...
		if( empty ( $src ) || $src == 'image_not_specified' ):
			...
			$output .= '<img src="'.$src.'" title="'.get_the_title().'" alt="'.get_the_title().'" /><div class="staff-image-hover"></div>';
			$output .= '</div>';	
...
function shortcode_tm_statictext($atts, $content = null){
	...
	$output .= '<div class="custom-text-container '.$classname.'" style="width:'.$width.';margin:'.$margin.';">';
...
function shortcode_progressbar($atts, $content = null)
{
	extract(shortcode_atts(array(
		...
		if($style == 4):			
			$output .=	'<small class="progress_detail" style="width:'.$value.'%;">';
			$output .= do_shortcode($content);
			...
			$output .= '<div class="active_progresbar" style="color:#'.$color.'" data-value="'.$value.'" data-percentage-value="'.$value.'">';
			$output .=	'<small class="progress_detail" style="color:#'.$color.';width:'.$value.'%;">';
			$output .= do_shortcode($content);
			if($show_percentage == 'yes') { 
				$output .= '<span class="tm_progress_label">'.$value.'%</span>';
			} 
...
function shortcode_banner($atts, $content = null)
{
	extract(shortcode_atts(array(
		...
		$output = '';
		$output .='<div class="tm_banner '.$class.'">';
		$output .='<div class="tm_banner_inner" style="margin:'.$margin.';">';
		$output .='<a href="'.$link_url.'" target="_Blank">';
		$output .='<img src="'.$get_imagepath.'" alt="" /><span></span>';	
		$output .='</a>';
		$output .='</div>';
		$output .='<div class="banner_content">';	
		if(!empty($title))
		$output .='<div class="title">'.$title.'</div>';
		$output .='<div class="smalltext">';
		if(!empty($subtext1))
		$output .='<h2 class="subtext1">'.$subtext1.'</h2>';
		if(!empty($subtext2))
		$output .='<h2 class="subtext2">'.$subtext2.'</h2>';
		if(!empty($subtext3))
		$output .='<h2 class="subtext3">'.$subtext3.'</h2>';
		$output .='</div>';  /* End smalltext */
		if(!empty($linktext))
		$output .='<div class="linktext"><a href='.$link_url.'>'.$linktext.'</a></div>';
		...
		if($type == 'slider'):
			...
			while (have_posts()) : the_post();
				...
				$output .= '<div class="product-block single-portfolio '.$class.' '.$layout.'">';
					...
					$output .= '<div class="portfolio-description">'.arise_portfolio_excerpt(100).'</div>';				
				$output .= '</div>';
				$output .= '</div>';
				$i++;
			endwhile;
...
$output .= '</div></div>';
		...
		$output .=  arise_portfolio_excerpt($excerpt_length);
		$output .= '</li>';
		$i++;
	}
	$output .= '</ul>';
	$output .= arise_shortcode_paging_nav();  
	$output .= '</div>';	  
	wp_reset_query();
	return $output;
}
...
function shortcode_about($atts, $content = null)
{
	extract(shortcode_atts(array(					
		...
		$output = '';
		$output .='<div class="tm_about" style="padding:'.$padding.'">';
		$output .='<div class="tm_about_inner image-'.$image_align.'">';
			if(!empty($image)):		
				$output .='<div class="about_image three_fifth">';
				$output .='<img src="'.$image.'" alt="" />';
				$output .='</div>';
			endif;
			if(!empty($image))
				$output .='<div class="about_content two_fifth">';
			else
...
{	
   extract(shortcode_atts(array(
	...
	$variables = '';
		...
		if($hover == "yes"):
			$output .='<li class="banner '.$classname.' animated " data-animated="'.$animation.'" style="'.$variables.';"><a href="'.$link_url.'" target="_blank"><img  data-animated="fadeInLeft" src="'.$image.'" alt=""/>
	   			<span class="hover_glass"> </span> </a></li>';	
		else :
		  $output .='<li class="banner animated '.$classname.'" data-animated="'.$animation.'" style="'.$variables.';"><a href="'.$link_url.'" target="_blank"><img  data-animated="fadeInLeft" src="'.$image.'" alt=""/>
	   			</a></li>';	
		endif;
	return $output;
}
...
function shortcode_one_half($atts, $content = null){
	...
	$output .= '<div class="one_half_inner content_inner '.$align.'" style="text-align:'.$txtalign.';margin:'.$margin.';width:'.$content_width.';">';
	$output .=	do_shortcode($content);
	$output .= '</div>';
	$output .= '</div>';
	return $output;
...
function shortcode_one_fourth($atts, $content = null){
	...
	$output = '';
	$output .= '<div class="one_fourth '.$class.'">';	
...
function shortcode_static_text($atts, $content = null){
	...
	$output = '';
	$output .= '<div class="static-text-container '.$align.'" style="margin:'.$margin.';padding:'.$padding.';">';
...
$src = mr_image_resize($image, 355, 212, true, 't', false);
if( empty ( $src ) || $src == 'image_not_specified' ):
$src = get_template_directory_uri()."/images/megnor/placeholder.png";
$src = mr_image_resize($src, 355, 212, true, 't', false);	
endif;
$output .= '<div class="item container '.$class.'">';
$output .= '<div class="container-inner">';
$output .= '<div class="post-image">';
$output .= '<div class="post-date"></i><div class="day">'.$post_day.'</div><div class="month">'.$post_month.'</div>, <div class="year">'.$post_year.'</div> </div>';	
$output .= '<img src="'.$src.'" title="'.get_the_title().'" alt="'.get_the_title().'" />';
...
$output .= '<div class="post-description">'.arise_blog_post_excerpt(13).'</div>';	
$output .= '<div class="post-author"><span>By</span> - '.get_the_author().'</div>';
$output .= '</div></div></div>';
endif;
...
$src = mr_image_resize($image, 461, 463, true, 't', false);
if( empty ( $src ) || $src == 'image_not_specified' ):
$src = get_template_directory_uri()."/images/megnor/placeholder.png";
$src = mr_image_resize($src, 355, 212, true, 't', false);	
endif;
...
$shorttitle = substr(get_the_title('','',FALSE),0,50);
$output .= '<div class="post-title"><a href="'.get_permalink().'" title="'.get_the_title().'">'.$shorttitle.'</a></div>';
$output .= '<div class="post-description">'.arise_blog_post_excerpt(50).'</div>';	
$output .= '<div class="raedmore"><a href="'.get_permalink().'" >Read More..</a></div>';
$output .= '<div class="post-author">';
$output .= '<div class="author"><span>By</span> - '.get_the_author().'</div>';
$output .= '<div class="post-date"></i><div class="day">'.$post_day.'</div><div class="month">'.$post_month.'</div>, <div class="year">'.$post_year.'</div> </div>';	
$output .= '</div></div>';
$output .= '</div></div></div>';
endif;
	...
	while ( $blog_array->have_posts() ) : $blog_array->the_post();
		...
		if( empty ( $src ) || $src == 'image_not_specified' ):
			...
			$output .= '<div class="container-inner">';
				...
				$shorttitle = substr(get_the_title('','',FALSE),0,50);
				$output .= '<div class="post-title"><a href="'.get_permalink().'" title="'.get_the_title().'">'.$shorttitle.'</a></div>';
				$output .= '<div class="post-description">'.arise_blog_post_excerpt(18).'</div>';				
	...
	while ( $news_array->have_posts() ) : $news_array->the_post();
		...
		if( empty ( $src ) || $src == 'image_not_specified' ):
			...
			$output .= '<div class="single-post">';
				...
				$shorttitle = substr(get_the_title('','',FALSE),0,50);
				$output .= '<div class="post-title"><a href="'.get_permalink().'" title="'.get_the_title().'">'.$shorttitle.'</a></div>';
				$output .= '<div class="post-description">'.arise_blog_post_excerpt(25).'</div>';
...
function tm_mapshortcode( $atts, $content = null )
{
	...
	$output .= '<div class="contactmap-inner">';	
	$output .= '
		<div id="map'.$id.'" class="google-map '.$class.'" '.$height.'></div>
        <script type="text/javascript" src="http://maps.google.com/maps/api/js?sensor=false&key=AIzaSyCdLv9Ac5yXVkfCLDzIB0EgRzanvkAaxTo"></script>
...
function shortcode_cms($atts, $content = null)
{
	extract(shortcode_atts(array(
			'title' =>  '',
			'child_title' =>  '',
	 ...
	 $output .='<div class="tm_cms" style="'.$variables.');">';
			$output .='<div class="parrellex_content">';
	 		$output .='<div class="pa-border"></div>';
			if(!empty($title))
			$output .='<div class="cms_title">'.$title.'</div>';
			if(!empty($child_title))
			$output .='<div class="cms_child_title">'.$child_title.'</div>';
			$output .='<div class="pa-border"></div>';
			if(!empty($subtitle) || !empty($subtitle1) || !empty($subtitle2) || !empty($subtitle3))
			$output .='<div class="subtext">';
			$output .='<div class="cms_subtitle">'.$subtitle.'</div>';
			$output .='<div class="cms_subtitle1">'.$subtitle1.'</div>';
			$output .='<div class="cms_subtitle2">'.$subtitle2.'</div>';
			$output .='</div>';
			if(!empty($content))
			$output .='<div class="cms_description">'.do_shortcode($content).'</div>';
			$output .='<div class="cms_button"><a href='.$link_url.'>'.$link_text.'</a></div>';
			$output .='</div>';
			if(!empty($src)){
			$output .='<div class="para-img">';
			$output .='<img src='.$src.' />';
			$output .='</div>';
			}
	$output .='</div>';		
...
add_shortcode('tm_cms', 'shortcode_cms');
